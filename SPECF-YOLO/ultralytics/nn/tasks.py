...
from ultralytics.nn.modules.fsda_2025 import *
from ultralytics.nn.modules.fsas_2025_ceshi import *
from ultralytics.nn.modules.CBL_2025 import *
...
#DetectionModel中调用parse_model，本方法用来调整f、n、r、arg，返回解析参数后的模型结构供上文方法
#赋值
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if isinstance(c2,list) and not two_backbone:
            m_=m
            m_.backbone=True
        else:
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in ('classify', 'classifier', 'cls', 'fc'):
            return 'classify'
        if m == ('detect','detect_dbb'):
            return 'detect'
        if m == 'segment':
            return 'segment'
        if m == 'pose':
            return ('pose','pose_dbb')
        else:
            return 'detect'
